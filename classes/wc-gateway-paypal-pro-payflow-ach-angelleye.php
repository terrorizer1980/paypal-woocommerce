 ...
 */
class WC_Gateway_PayPal_Pro_PayFlow_ACH_AngellEYE extends WC_Payment_Gateway_CC {
    ...
    function __construct() {
        $this->id = 'paypal_pro_payflow_ach';
        $this->method_title = __('PayPal Payments Pro 2.0 (PayFlow) ACH', 'paypal-for-woocommerce');
        ...
        $this->icon = apply_filters('woocommerce_paypal_pro_payflow_ach_icon', $this->icon);
        ...
        add_action('woocommerce_api_wc_gateway_paypal_pro_payflow_ach_angelleye', array($this, 'handle_3dsecure'));

        add_filter('woocommerce_credit_card_form_fields', array($this, 'angelleye_paypal_pro_payflow_ach_credit_card_form_fields'), 10, 2);
        add_filter('woocommerce_settings_api_sanitized_fields_' . $this->id, array($this, 'angelleye_paypal_pro_payflow_ach_encrypt_gateway_api'), 10, 1);
        ...
        if ($this->avs_cvv2_result_admin_email) {
            if (!has_filter('woocommerce_email_before_order_table', array($this, 'angelleye_paypal_pro_payflow_ach_email_instructions'))) {
                add_action('woocommerce_email_before_order_table', array($this, 'angelleye_paypal_pro_payflow_ach_email_instructions'), 10, 3);
            }
        }
    ...
    public function add_log($message, $level = 'info') {
        if ($this->debug) {
            if (version_compare(WC_VERSION, '3.0', '<')) {
                ...
                $this->log->add('paypal_pro_payflow_ach', $message);
            } else {
                if (empty($this->log)) {
                    $this->log = wc_get_logger();
                }
                $this->log->log($level, sprintf(__('PayPal for WooCommerce Version: %s', 'paypal-for-woocommerce'), VERSION_PFW), array('source' => 'paypal_pro_payflow_ach'));
                $this->log->log($level, sprintf(__('WooCommerce Version: %s', 'paypal-for-woocommerce'), WC_VERSION), array('source' => 'paypal_pro_payflow_ach'));
                $this->log->log($level, 'Test Mode: ' . $this->testmode, array('source' => 'paypal_pro_payflow_ach'));
                $this->log->log($level, $message, array('source' => 'paypal_pro_payflow_ach'));
            }
        }
    }
...
of the user authorized to process transactions. Otherwise, leave this field blank.', 'paypal-for-woocommerce'),
            ...
            'debug' => array(
                ...
                'default' => 'no',
                'description' => sprintf(__('Log PayPal events inside <code>%s</code>', 'paypal-for-woocommerce'), wc_get_log_file_path('paypal_pro_payflow_ach')),
            ),
            'is_encrypt' => array(
    ...
    public function admin_options() {
        ...
        echo $this->angelleye_paypal_pro_payflow_ach_reference_transaction_notice();
        ?>
        ...
        <script type="text/javascript">
            ...
            ?> jQuery('#woocommerce_paypal_pro_payflow_ach_enable_tokenized_payments').prop("disabled", true);
               jQuery('#woocommerce_paypal_pro_payflow_ach_enable_tokenized_payments').prop('checked', false); 
            <?php
            } ?>
            jQuery('#woocommerce_paypal_pro_payflow_ach_payment_action').change(function () {
                if (this.value === 'Authorization') {
                    jQuery('#woocommerce_paypal_pro_payflow_ach_payment_action_authorization').closest('tr').show();
                    jQuery('#woocommerce_paypal_pro_payflow_ach_pending_authorization_order_status').closest('tr').show();
                    jQuery('#woocommerce_paypal_pro_payflow_ach_default_order_status').closest('tr').hide();
                } else {
                    jQuery('#woocommerce_paypal_pro_payflow_ach_payment_action_authorization').closest('tr').hide();
                    jQuery('#woocommerce_paypal_pro_payflow_ach_pending_authorization_order_status').closest('tr').hide();
                    jQuery('#woocommerce_paypal_pro_payflow_ach_default_order_status').closest('tr').show();
                }
            }).change();
            jQuery('#woocommerce_paypal_pro_payflow_ach_testmode').change(function () {
                var sandbox = jQuery('#woocommerce_paypal_pro_payflow_ach_sandbox_paypal_partner, #woocommerce_paypal_pro_payflow_ach_sandbox_paypal_vendor, #woocommerce_paypal_pro_payflow_ach_sandbox_paypal_user, #woocommerce_paypal_pro_payflow_ach_sandbox_paypal_password').closest('tr'),
                        production = jQuery('#woocommerce_paypal_pro_payflow_ach_paypal_partner, #woocommerce_paypal_pro_payflow_ach_paypal_vendor, #woocommerce_paypal_pro_payflow_ach_paypal_user, #woocommerce_paypal_pro_payflow_ach_paypal_password').closest('tr');
            ...
            jQuery('#woocommerce_paypal_pro_payflow_ach_send_items').change(function () {
                var payflow_subtotal_mismatch_behavior = jQuery('#woocommerce_paypal_pro_payflow_ach_subtotal_mismatch_behavior').closest('tr');
                if (jQuery(this).is(':checked')) {
                    ...
                    paypal_pro_payflow_ach_ach-card-number
            jQuery('#woocommerce_paypal_pro_payflow_ach_enable_3dsecure').change(function () {
                var dsecure_type = jQuery('#woocommerce_paypal_pro_payflow_ach_threedsecure_type, #woocommerce_paypal_pro_payflow_ach_liability_shift').closest('tr');
                var centinel = jQuery('#woocommerce_paypal_pro_payflow_ach_centinel_pid, #woocommerce_paypal_pro_payflow_ach_centinel_mid, #woocommerce_paypal_pro_payflow_ach_centinel_pwd').closest('tr');
                if (jQuery(this).is(':checked')) {
                    dsecure_type.show();
                    if (jQuery('#woocommerce_paypal_pro_payflow_ach_threedsecure_type').val() === 'cardinalcommerce') {
                        centinel.show();
                    }
                } else {
                    dsecure_type.hide();
            ...
            jQuery('#woocommerce_paypal_pro_payflow_ach_threedsecure_type').change(function () {
                var centinel = jQuery('#woocommerce_paypal_pro_payflow_ach_centinel_pid, #woocommerce_paypal_pro_payflow_ach_centinel_mid, #woocommerce_paypal_pro_payflow_ach_centinel_pwd').closest('tr');
                if (this.value === 'cardinalcommerce') {
                    if( jQuery('#woocommerce_paypal_pro_payflow_ach_enable_3dsecure').is(':checked')) {
                        centinel.show();
                    }
                } else {
                    centinel.hide();
    ...
    function process_payment($order_id) {
        ...
        if (!empty($_POST['wc-paypal_pro_payflow_ach-payment-token']) && $_POST['wc-paypal_pro_payflow_ach-payment-token'] != 'new') {
            $this->enable_3dsecure = false;
        }
        ...
        if ($this->enable_3dsecure) {
            ...
            if ($this->threedsecure_type == 'cardinalcommerce') {
                ...
                $PayPalResult = $this->PayPal->ProcessTransaction(apply_filters('angelleye_woocommerce_paypal_pro_payflow_ach_process_transaction_request_args', $PayPalRequestData));
    ...
    function do_payment($order, $card_number, $card_exp, $card_csc, $centinel = null) {
        ...
        try {
            ...
            if (!empty($_POST['paypal_pro_payflow_ach-card-cardholder-first'])) {
                $firstname = wc_clean($_POST['paypal_pro_payflow_ach-card-cardholder-first']);
            } else {
                $firstname = version_compare(WC_VERSION, '3.0', '<') ? $order->billing_first_name : $order->get_billing_first_name();
            }

            if (!empty($_POST['paypal_pro_payflow_ach-card-cardholder-last'])) {
                $lastname = wc_clean($_POST['paypal_pro_payflow_ach-card-cardholder-last']);
            } else {
            ...
            if (!empty($_POST['wc-paypal_pro_payflow_ach-payment-token']) && $_POST['wc-paypal_pro_payflow_ach-payment-token'] != 'new') {
                $token_id = wc_clean($_POST['wc-paypal_pro_payflow_ach-payment-token']);
    ...
    public function paypal_for_woocommerce_paypal_pro_payflow_ach_credit_card_form_expiration_date_selectbox() {
        $form_html = "";
        ...
        $form_html .= '<select name="paypal_pro_payflow_ach_card_expiration_month" id="cc-expire-month" class="woocommerce-select woocommerce-cc-month mr5">';
        ...
        $form_html .= '</select>';
        $form_html .= '<select name="paypal_pro_payflow_ach_card_expiration_year" id="cc-expire-year" class="woocommerce-select woocommerce-cc-year ml5">';
    ...
    public function validate_fields() {
        ...
        if (isset($_POST['wc-paypal_pro_payflow_ach-payment-token']) && 'new' !== $_POST['wc-paypal_pro_payflow_ach-payment-token']) {
            $token_id = wc_clean($_POST['wc-paypal_pro_payflow_ach-payment-token']);
            $token = WC_Payment_Tokens::get($token_id);
        ...
        $card_number = isset($_POST['paypal_pro_payflow_ach-card-number']) ? wc_clean($_POST['paypal_pro_payflow_ach-card-number']) : '';
        $card_cvc = isset($_POST['paypal_pro_payflow_ach-card-cvc']) ? wc_clean($_POST['paypal_pro_payflow_ach-card-cvc']) : '';
        $card_exp_year = isset($_POST['paypal_pro_payflow_ach_card_expiration_year']) ? wc_clean($_POST['paypal_pro_payflow_ach_card_expiration_year']) : '';
        $card_exp_month = isset($_POST['paypal_pro_payflow_ach_card_expiration_month']) ? wc_clean($_POST['paypal_pro_payflow_ach_card_expiration_month']) : '';
    ...
    public function angelleye_paypal_pro_payflow_ach_credit_card_form_fields($default_fields, $current_gateway_id) {
        if ($current_gateway_id == $this->id) {
            $fields = array(
                ...
                'card-expiry-field' => $this->paypal_for_woocommerce_paypal_pro_payflow_ach_credit_card_form_expiration_date_selectbox(),
    ...
    public function get_posted_card() {
        ...
        if ($card_type == 'amex') {
            if (WC()->countries->get_base_country() == 'CA' && get_woocommerce_currency() == 'USD' && apply_filters('angelleye_paypal_pro_payflow_ach_amex_ca_usd', true, $this)) {
    ...
    public function angelleye_paypal_pro_payflow_ach_encrypt_gateway_api($settings) {
    ...
    public function angelleye_paypal_pro_payflow_ach_email_instructions($order, $sent_to_admin, $plain_text = false) {
        $payment_method = version_compare(WC_VERSION, '3.0', '<') ? $order->payment_method : $order->get_payment_method();
        if ($sent_to_admin && 'paypal_pro_payflow_ach' === $payment_method) {
            // Store source in the order
            $order_id = version_compare(WC_VERSION, '3.0', '<') ? $order->id : $order->get_id();
            $this->angelleye_load_paypal_payflow_class($this->gateway, $this, $order_id);
            $old_wc = version_compare(WC_VERSION, '3.0', '<');
    ...
    public function angelleye_paypal_pro_payflow_ach_reference_transaction_notice() {
        if (class_exists('AngellEYE_Utility')) {
            if (AngellEYE_Utility::is_display_angelleye_paypal_pro_payflow_reference_transaction_notice($this) == true) {
                echo '<div class="error"><p>' . sprintf(__("If using %s with Woo Token Payments (including the use of Woo Subscriptions) you will need to <a target='_blank' href='https://www.angelleye.com/paypal-woocommerce-subscriptions/'>enable Reference Transactions</a> in your PayPal/PayFlow Manager. | <a href=%s>%s</a>", 'paypal-for-woocommerce'), $this->method_title, '"' . esc_url(add_query_arg("ignore_paypal_pro_payflow_ach_reference_transaction_notice", 0)) . '"', __("Hide this notice", 'paypal-for-woocommerce')) . '</p></div>';
            }
        }
    }
